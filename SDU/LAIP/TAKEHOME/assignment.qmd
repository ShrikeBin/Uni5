...
$$
\begin{aligned}
& \max && \sum_{j=1}^{N} c_j x_j \\[2mm]
& && \sum_{j=1}^{N} x_j \, \widehat{R}_j \ge \alpha + \epsilon (\beta - \alpha), \\[1mm]
& && \sum_{j=1}^{N} x_j = 1, \\[1mm]
...
Then this should work (because we need to make $\gamma$ only bigger than the smallest one)
...
& && \sum_{j=1}^{N} x_j \, \widehat{R}_j \ge \alpha + \epsilon (\beta - \alpha), \\[1mm]
& && \sum_{j=1}^{N} x_j = 1, \\[1mm]
& && \sum_{j=1}^{N} \theta_j \leq M, \\[1mm]
& && x_j \leq \theta_j \quad j = 1,\dots,N \\\\
& && x_j \ge 0 \quad j = 1,\dots,N \\
& &&\theta_j \in \left\{\mathbf{0},\mathbf{1}\right\} \quad j = 1,\dots,N \\
\end{aligned}
$$

Here $\theta_j = 0$ makes corresponding $x_j = 0$ and
$\theta_j = 1$ makes corresponding $x_j \in [0, 1]$ \newline
...
We can do that by adding another constraint that, when corresponding $\theta_j \neq 0$ will enforce $x_j \in [v, 1]$
