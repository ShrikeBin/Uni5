...
c_j &= \frac{1}{T}\sum_{t=1}^T{(r_jt - \hat{R_j})}\\\\
\alpha &= \max\Big( 0, \min_j \hat{R}_j \Big)\\\\
\beta &= \max_j \hat{R}_j\\\\
\hat{R} &= \sum_{j=1}^N{x_jR_{j}} =\\
&= \text{reward}\\\\
\widehat{\text{MAD}} &= \frac{1}{T}\sum_{t=1}^T{\left[\sum_{j=1}^N{x_j(r_jt - \hat{R_j})}\right]} = \\
&=\sum_{j=1}^N{x_j\left[\frac{1}{T}\sum_{t=1}^T{(r_jt - \hat{R_j})}\right]} =\\
&= \sum_{j=1}^{N} c_j x_j=\\
&= \text{risk}\\\\
\end{aligned}
\end{matrix}
...
\begin{aligned}
& \max && \sum_{j=1}^{N} c_j x_j \\
& && \sum_{j=1}^{N} x_j \, \widehat{R}_j \ge \alpha + \epsilon (\beta - \alpha), \\
& && \sum_{j=1}^{N} x_j = 1, \\
...
The soluton is based on provided template, adjusted to use **mip** and my model (additional $c_j$ for simpler calculations):
